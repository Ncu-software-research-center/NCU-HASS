...
echo POWERFAILURE-started $(date '+%Y-%m-%d %H:%M:%S,%3N');sudo kill -9 $(ps aux | grep -E '^libvirt+.*instance-00000020' | tr -s ' ' | cut -d ' ' -f 2); echo POWERFAILURE-finished $(date '+%Y-%m-%d %H:%M:%S,%3N')