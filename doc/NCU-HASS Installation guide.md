...
###### reference : [HASS m3.1 Installation guide](https://hackmd.io/lsNdRYcHSYWMAE8eTM42NA?view#HASS-Installation-guide)
...
#### Modify /etc/cinder/cinder.conf on Controller and Blockstorage
[DEFAULT]
...
...
1. copy file
```
# Copy [HA Admin] menu on Horizon
sudo cp -r HASS-4.1/controller_node/Dashboards/haAdmin \
/usr/share/openstack-dashboard/openstack_dashboard/dashboards/

# Copy [HA Project] menu on Horizon
sudo cp -r HASS-4.1/controller_node/Dashboards/haProject \
/usr/share/openstack-dashboard/openstack_dashboard/dashboards/

# Enable [HA Admin] menu on Horizon
sudo cp HASS-4.1/Controller_node/Dashboards/enabled/_22_haAdmin.py \
/usr/share/openstack-dashboard/openstack_dashboard/enabled/_22_haAdmin.py

# Enable [HA Project] menu on Horizon
sudo cp HASS-4.1/controller_node/Dashboards/enabled/_21_haProject.py \
/usr/share/openstack-dashboard/openstack_dashboard/enabled/_21_haProject.py

# Enable [REST] function on Horizon
sudo cp HASS-4.1/Controller_node/Dashboards/REST \
/usr/share/openstack-dashboard/openstack_dashboard/REST

```

3. Put `HASS-4.1/controller_node/HASS/` in `/home/localadmin/`
...
[mysql]
mysql_ip = controller
mysql_username = hass # HASSDB_USER
...
##### 1. Put `HASS-4.1/compute_node/HASS/` in `/home/localadmin/`
...
Create `/etc/systemd/system/detectionagent.service` as `root` on compute host with following content
...
[Service]
ExecStart=/usr/bin/python /home/localadmin/HASS-4.1/compute_node/HASS/DetectionAgent.py
WorkingDirectory=/home/localadmin/HASS-4.1/compute_node/HASS/
...
After refoot
```
sudo systemctl status detectionagent
```
...
```
:::info
You need to remount it to read-only mode if `/dev/xxx is mounted in rw` pop up. 

For more information see [How to umount / ? It's busy](https://superuser.com/questions/429199/how-to-umount-its-busy)
:::
